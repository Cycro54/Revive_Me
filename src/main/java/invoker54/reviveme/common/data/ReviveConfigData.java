...
public class ReviveConfigData {
    ...
    public static EffectInstance EffectFromString(String effectString) {
        try {
            ...
            return new EffectInstance(effect, ticks, tier, false, isVisible);
        }
        catch (Exception e){
    ...
    public void revivePlayer(PlayerEntity fallen, boolean isCommand, Entity reviver, InvoText reviveText){
        ...
        if (getFallenPenaltyTimer() != 0) fallen.addEffect(new EffectInstance(EffectInit.FALLEN_EFFECT, (int) (fallenPenaltyTimer * 20), cap.getPenaltyMultiplier(), false,
                !ReviveMeConfig.hidePenaltyTimerEffect, true));

        if (this.reviveEffects != null) {
            for (EffectInstance instance : reviveEffects) {
                fallen.addEffect(new EffectInstance(instance.getEffect(), instance.getDuration(), instance.getAmplifier(), false, instance.isVisible()));
            }
        }
