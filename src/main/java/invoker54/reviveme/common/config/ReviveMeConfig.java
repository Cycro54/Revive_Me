...
public final class ReviveMeConfig {
    ...
    public static void bakeConfig(){
        ...
        if (selfReviveOptions.isEmpty()) selfReviveOptions.add(FallenCapability.SELFREVIVETYPE.CHANCE);
        ...
        Stream<Map.Entry<ResourceKey<MobEffect>, MobEffect>> negativeEffects = ForgeRegistries.MOB_EFFECTS.getEntries().stream().filter(
        ...
        if (harmfulEffectsBlackList.contains("//")){
            harmfulEffects = negativeEffects.filter(a -> harmfulEffectsBlackList.stream()
                            .anyMatch(b -> !b.isEmpty() && a.getKey().location().toString().contains(b)))
                    .map(Map.Entry::getValue).collect(Collectors.toList());
        }
        else {
            harmfulEffects = negativeEffects.filter(a -> harmfulEffectsBlackList.stream()
                            .anyMatch(b -> b.isEmpty() || a.getKey().location().toString().contains(b) == false))
                    .map(Map.Entry::getValue).collect(Collectors.toList());
        }
    ...
    public static CompoundTag serialize(){
        ...
        for (FallenCapability.SELFREVIVETYPE reviveType : selfReviveOptions) {
//            LOGGER.error("Whats the class thing?? " + reviveType.getClass());
    ...
    public static void deserialize(CompoundTag mainTag){
        ...
        for (String reviveType : allSelfReviveOptions) selfReviveOptions.add(FallenCapability.SELFREVIVETYPE.valueOf(reviveType));
        //Max Self Revives
        ...
        public CommonConfig(ForgeConfigSpec.Builder builder) {
            ...
            harmfulEffectsBlackList = builder.comment("List of harmful effects that shouldn't be chosen. Add \"//\" to make it a whitelist instead. You can also use parts of an effect Usage: (ModId:PotionEffect) 'minecraft:slowness' or 'slowness' or 'minecraft'")
                    .defineList("Harmful_Effects_Blacklist", Arrays.asList("minecraft:bad_omen", "minecraft:unluck", "minecraft:instant_damage", "reviveme:kill_revive_effect"),
                            a -> !a.toString().isEmpty() && ((a.equals("//")) || ForgeRegistries.MOB_EFFECTS.getEntries().stream().filter((effect) -> effect.getValue().getCategory() == MobEffectCategory.HARMFUL)
                                    .map(e -> e.getKey().location().toString()).collect(Collectors.toList()).contains(a)));
