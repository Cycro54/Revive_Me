...
//        LOGGER.warn("What's pose: " + event.player.getForcedPose());
        ...
        switch (ReviveMeConfig.timerType){
            ...
            case DEATH:
                ...
                break;
            case REVIVE:
                cap.incrementReviveCount(cap.canSelfRevive() ? null : event.player);
                ReviveMeConfig.configReviveData.revivePlayer(event.player, false, event.player, "countdown");
                break;
        }
    ...
    public static void TickProgress(TickEvent.PlayerTickEvent event) {
        ...
        PlayerEntity otherPlayer = event.player.getServer().getPlayerList().getPlayer(cap.getOtherPlayer());
