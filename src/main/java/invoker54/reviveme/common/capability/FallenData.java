    ...
    public void removeOriginalEffects(boolean reset) {
        ...
        for (var effect : new ArrayList<>(this.player.getActiveEffectsMap().keySet())) {
            try {
                ...
                Identifier modLocation = BuiltInRegistries.MOB_EFFECT.getKey(effect.value());
                String modID = modLocation == null ? "null" : modLocation.getNamespace();
                LOGGER.error("[Revive Me!] Some mod doesn't want this effect removed: " + effect.value().getDisplayName().getString() + " <- mod: " + modID);
            }
        }
...
//                if (optional.isEmpty()) continue;
        ...
        this.playerReviveCount = tag.getInt(PLAYER_REVIVE_COUNT_INT).get();
...
//            if (optional.isPresent()) this.reviveStack = optional.get();
        ...
        this.maxOverkill = tag.getDouble(MAX_OVERKILL_DOUBLE).get();
        this.currentOverkill = tag.getDouble(CURRENT_OVERKILL_DOUBLE).get();
