...
import static invoker54.reviveme.client.event.FallScreenEvent.EFFECT_BACKGROUND;
...
public class FallenItemScreenEvent {
    ...
    private static final String itemLangDirectory = "revive_me.fallenScreen.items.";
...
//                .inflate(-1, -0.5F), TextUtil.txtAlignment.MIDDLE);
        ...
        previousTick = ClientUtil.getMinecraft().player.level().getGameTime() + FallScreenEvent.getPartialTicks();
    }
    ...
    public static void renderItem(InvoZone myZone, Pair<ItemStack, ReviveItemData> pair, PoseStack stack, boolean isMain){
        ...
        if (isMain && VanillaKeybindHandler.useHeld) {
            ...
            if (!pair.getKey().has(DataComponents.CUSTOM_NAME) && data != null && !data.getDisplayName().getString().isEmpty()) nameText = data.getDisplayName();
            if (data == null){
    ...
    public static List<BiConsumer<PoseStack, InvoZone>> getDescriptionPages(ReviveItemData data, InvoZone textZone){
        ...
        for (MutableComponent comp : textList){
            consumerList.add((stack, bodyZone) -> {
                ...
                int lineCount = (int) Math.ceil(ClientUtil.getMinecraft().font.width(comp)/(float)spacePerLine);
...
//                ClientUtil.blitColor(stack, bodyZone, blackFadeColor);
    ...
    public static void switchReviveScreens(){
        ...
        startTransition = ClientUtil.getMinecraft().level.getGameTime();
    ...
    public static double getSwitchPercentage() {
        ...
        double percentage = ((ClientUtil.getMinecraft().level.getGameTime() + FallScreenEvent.getPartialTicks()) - startTransition) / range;
