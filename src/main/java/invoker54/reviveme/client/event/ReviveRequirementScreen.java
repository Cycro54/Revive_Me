...
public class ReviveRequirementScreen {
    ...
    public static void registerRequirementScreen(RegisterGuiLayersEvent event) {
        ...
        event.registerAboveAll(makeResource("requirement_screen"), (guiGraphics, tracker) -> {
            ...
            renderRevivesLeft(guiGraphics, requirementZone, cap);
        });
    }

    public static void renderRequirements(GuiGraphicsExtractor stack, InvoZone requirementZone, FallenData cap, float penaltyTypeSize){
        ...
        ClientUtil.blit2DColor(stack, requirementZone, blackBg);

        chosenZone.setWidth(penaltyTypeSize).setHeight(penaltyTypeSize).center(requirementZone.copy().splitWidth(2, 1));

        if (itemData != null) {
            ClientUtil.blit2DItem(stack, chosenZone, ClientUtil.getPlayer().getMainHandItem());
        }
        ...
        TextUtil.render2DText(stack, penaltyText.getText(), false, 1,
        ...
        TextUtil.render2DText(stack, startTxt.getText(), true, 1,
                textZone.copy().inflate(-2,-2), TextUtil.txtAlignment.MIDDLE);

        TextUtil.render2DText(stack, arrowTxt.getText(), true, 1,
                textZone.shiftXY(textZone.width(),0).copy().inflate(-2,-2), TextUtil.txtAlignment.MIDDLE);

        TextUtil.render2DText(stack, endTxt.getText(), true, 1,
                textZone.shiftXY(textZone.width(),0).copy().inflate(-2,-2), TextUtil.txtAlignment.MIDDLE);


    }

    public static void renderRevivesLeft(GuiGraphicsExtractor stack, InvoZone requirementZone, FallenData cap){
        int playerReviveCount = cap.getPlayerReviveCount(true);
        if (playerReviveCount == -1) return;
        ClientUtil.blit2DColor(stack, requirementZone.inflate(5, 0), blackBg);

        InvoText pluralText = playerReviveCount == 1 ? reviveCountSingleText : reviveCountMultipleText;

        TextUtil.render2DText(stack, InvoText.translate("revive_me.fall_plate.revive_count", InvoText.literal(cap.getPlayerReviveCount(true) + "")
