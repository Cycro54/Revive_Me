...
public class ReviveRequirementScreen {
    ...
    public static void renderRequirementScreen(RenderGameOverlayEvent.Post event) {
        ...
        if (getPlayer().isShiftKeyDown()) return;
        FallenCapability cap = FallenCapability.get((LivingEntity) mC.crosshairPickEntity);
        ...
        //Penalty txt
        float penaltyAmount = cap.getPenaltyAmount(mC.player);
        if (ReviveMeConfig.penaltyType == FallenCapability.PENALTYPE.EXPERIENCE){
            float totalExperience = ReviveMathUtil.getExperienceFromLevel(mC.player);
            float prevLevel = mC.player.experienceLevel + mC.player.experienceProgress;
        ...
        penaltyText.withStyle(true, InvoTextFormat.filter(TextFormatting.BOLD))
                .withStyle(false, InvoTextFormat.filter(cap.hasEnough(mC.player) ? TextFormatting.GREEN : TextFormatting.RED));
        ...
        float endAmount;
        if (itemData == null){
            startAmount = (int) Math.round(cap.countReviverPenaltyAmount(mC.player));
            endAmount = Math.round(startAmount - penaltyAmount);

            if (ReviveMeConfig.penaltyType == FallenCapability.PENALTYPE.EXPERIENCE){
                float totalExperience = ReviveMathUtil.getExperienceFromLevel(mC.player.experienceLevel, mC.player.experienceProgress);
                totalExperience = (totalExperience - cap.getPenaltyAmount(mC.player));
                endAmount = ReviveMathUtil.getLevelFromExperience((int) totalExperience);
            }
        }
        else {
            startAmount = itemData.getItemCount(mC.player, mC.player.getMainHandItem());
            endAmount = startAmount - itemData.getCountRequired();
        }

        InvoText startTxt = InvoText.literal("" + startAmount)
                .withStyle(true, InvoTextFormat.filter(TextFormatting.BOLD, TextFormatting.GREEN));
        ...
        InvoText endTxt = InvoText.literal(df.format(endAmount))
                .withStyle(true, InvoTextFormat.filter(TextFormatting.BOLD,TextFormatting.RED));
