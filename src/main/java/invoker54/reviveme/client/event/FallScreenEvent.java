...
//    public static final Identifier EFFECT_BACKGROUND = Identifier.withDefaultNamespace("textures/gui/sprites/container/inventory/effect_background.png");
    ...
    public static void registerFallenScreen(RegisterGuiLayersEvent event) {
        ...
        event.registerAbove(VanillaGuiLayers.CHAT, makeResource("revive_button"), ((guiGraphics, tracker) ->
        {
            ...
            ClientUtil.blit2DColor(guiGraphics, toggleZone.copy().inflate(-1, -1), toggleFGColor.getRGB());
        ...
        event.registerAbove(VanillaGuiLayers.CHAT, makeResource("fallen_self_revive_screen"), (guiGraphics, tracker) -> {
            ...
            if (!FallenItemScreenEvent.isItemScreenActive || !isSwitchDone) {
                ...
                renderReviveOption(secondOption, guiGraphics, rightZone, cap, VanillaKeybindHandler.useHeld);
                //endregion
            }
        });
    ...
    public static void renderHeaderAndReviveCount(GuiGraphics guiGraphics, FallenData cap, InvoZone workZone){
        //Title text
        ...
        renderReviveCount(guiGraphics, reviveTextZone, mainCount, mainTypeText, reviveCountFullText);
        ...
        renderReviveCount(guiGraphics, reviveLeftZone, selfRevives, reviveCountSelfText, reviveCountShortText);

        InvoZone reviveRightZone = reviveLeftZone.copy().mirrorX(reviveTextZone.middleX());
        renderReviveCount(guiGraphics, reviveRightZone, playerRevives, reviveCountPlayerText, reviveCountShortText);
    }

    public static void  renderReviveCount(GuiGraphics guiGraphics, InvoZone renderZone, int reviveCount, InvoText typeText, InvoText mainText) {
        ...
        ClientUtil.blit2DColor(guiGraphics, renderZone, new Color(0, 0, 0, 150).getRGB());
        TextUtil.render2DText(guiGraphics, mainText.getText(), true, 1,
                renderZone.copy().inflate(-2, -2), TextUtil.txtAlignment.MIDDLE);
    }

    public static void renderReviveOption(int mouseButton, GuiGraphics guiGraphics, InvoZone workZone, FallenData cap, boolean beingHeld) {
        FallenData.SELFREVIVETYPE selfReviveType = cap.getSelfReviveOption(mouseButton);
        Minecraft inst = ClientUtil.getMinecraft();

        boolean shouldPass = true;
        ClientUtil.blit2DColor(guiGraphics, workZone, blackFadeColor);
        ...
        ClientUtil.blit2DColor(guiGraphics, mainZone, new Color(0, 0, 0, 128).getRGB());
        ...
        switch (selfReviveType) {
            case CHANCE: {
                ...
                TextUtil.render2DText(guiGraphics, chanceNumberTxt.getText(), true, 1, mainZone.copy()
            ...
            case RANDOM_ITEMS: {
                ...
                if (itemArrayList.isEmpty()){
                    shouldPass = false;
                    TextUtil.render2DText(guiGraphics, randomItemFalseTxt.deepCopy().withStyle(true,
                ...
                for (ItemStack sacrificeStack : itemArrayList) {
                    //Draw the background
                    ClientUtil.blit2DColor(guiGraphics, mainZone, new Color(0, 0, 0, 255).getRGB());
                    ...
                    //Draw the item
                    ClientUtil.blit2DItem(guiGraphics, randomItemImageZone, sacrificeStack);
                    ...
                    TextUtil.render2DText(guiGraphics, InvoText.literal("" + count).withStyle(true, InvoTextFormat.filter(ChatFormatting.BOLD, ChatFormatting.GREEN)).getText(),
                            true, 1, randomItemTxtZone, TextUtil.txtAlignment.MIDDLE);
                    randomItemTxtZone.shiftXY(randomItemTxtZone.width(), 0);

                    TextUtil.render2DText(guiGraphics, InvoText.literal(" -> ").withStyle(true, InvoTextFormat.filter(ChatFormatting.BOLD)).getText(),
                            true, 1, randomItemTxtZone, TextUtil.txtAlignment.MIDDLE);
                    randomItemTxtZone.shiftXY(randomItemTxtZone.width(), 0);

                    TextUtil.render2DText(guiGraphics, InvoText.literal("" + (count - (Math.round(Math.max(1,
            ...
            case KILL: {
                ...
                if (shouldPass) {
                    ...
                    TextUtil.render2DText(guiGraphics, killTxt2.withStyle(true, InvoTextFormat.filter(ChatFormatting.GOLD)).setArgs(
                    ...
                    TextUtil.render2DText(guiGraphics, killTxt3.withStyle(true, InvoTextFormat.filter(ChatFormatting.GOLD, ChatFormatting.BOLD)).getText(),
                            true, 1, mainZone.shiftXY(0, mainZone.height()).copy()
                                    .inflate(-3, -3), TextUtil.txtAlignment.MIDDLE);

                    TextUtil.render2DText(guiGraphics, killTxt4.withStyle(true, InvoTextFormat.filter(ChatFormatting.GOLD)).setArgs(InvoText.literal("" + seconds)
                                    .withStyle(true, InvoTextFormat.filter(ChatFormatting.RED)).getText()).getText(),
                            true, 1, mainZone.shiftXY(0, mainZone.height()).copy()
                                    .inflate(-3, -3), TextUtil.txtAlignment.MIDDLE);
                }
                else {
                    TextUtil.render2DText(guiGraphics, killFalseTxt.withStyle(true, InvoTextFormat.filter(ChatFormatting.RED)).getText(),
                            true, 0, mainZone.inflate(-4,-4), TextUtil.txtAlignment.MIDDLE);
                }
...
//                    LOGGER.warn("What's the id? " + effectId);
                    TextureAtlasSprite sprite = guiGraphics.guiSprites.getSprite(effectId);
                    ClientUtil.Image effectIMG = new ClientUtil.Image(sprite.atlasLocation(), sprite.getU0(),
                            (sprite.getU1() - sprite.getU0()), sprite.getV0(),
                            (sprite.getV1() - sprite.getV0()), 1, 1);
...
//                    guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, ambient ? EFFECT_BACKGROUND_AMBIENT_SPRITE : EFFECT_BACKGROUND_SPRITE, x, y, k, 32);
                    guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, EFFECT_BACKGROUND, (int) backgroundZone.x(), (int) backgroundZone.y(), (int) backgroundZone.width(), (int) backgroundZone.height());
//                    backgroundImg.render(guiGraphics);
                    ...
                    TextUtil.render2DText(guiGraphics, InvoText.literal(s).getText(), true, 1, textZone,
                            TextUtil.txtAlignment.LEFT);
                    textZone.setY(textZone.down()+2);
                    TextUtil.render2DText(guiGraphics, InvoText.literal(s1).withStyle(true, InvoTextFormat.filter(ChatFormatting.DARK_GRAY)).getText(),
            ...
            case EXPERIENCE: {
                ...
                ClientUtil.blit2DItem(guiGraphics, itemZone.inflate(-5,-5),
                ...
                if (ReviveMeConfig.minReviveXPLevel <= inst.player.experienceLevel) {
                    ...
                    TextUtil.render2DText(guiGraphics, InvoText.literal(inst.player.experienceLevel + "").withStyle(true, InvoTextFormat.filter(ChatFormatting.GREEN, ChatFormatting.BOLD))
                            .getText(), true, 1, mainZone.copy().inflate(-2, -2), TextUtil.txtAlignment.MIDDLE);

                    mainZone.shiftXY(mainZone.width(), 0);
                    TextUtil.render2DText(guiGraphics, InvoText.literal("->")
                            .getText(), true, 1, mainZone.copy().inflate(-2, -2), TextUtil.txtAlignment.MIDDLE);

                    mainZone.shiftXY(mainZone.width(), 0);
                    TextUtil.render2DText(guiGraphics, InvoText.literal(df.format(newLevel)).withStyle(true, InvoTextFormat.filter(ChatFormatting.RED, ChatFormatting.BOLD))
                            .getText(), true, 1, mainZone.copy().inflate(-2, -2), TextUtil.txtAlignment.MIDDLE);
                }
                else {
                    shouldPass = false;
                    ClientUtil.blit2DColor(guiGraphics, mainZone, new Color(0,0,0,230).getRGB());
                    TextUtil.render2DText(guiGraphics, experienceFalseTxt.setArgs(InvoText.literal(""+ReviveMeConfig.minReviveXPLevel)
            ...
            case NONE: {
                shouldPass = false;
                chosenTxt = chanceTxt;
                ClientUtil.blit2DItem(guiGraphics, mainZone.copy().setWidth(Math.min(mainZone.width(), mainZone.height()))
        ...
        if (!shouldPass) ClientUtil.blit2DColor(guiGraphics, progressZone, blackFadeColor);
        ...
        if (beingHeld) {
            ...
            ClientUtil.blit2DColor(guiGraphics, progressZone.copy().setHeight(fillPercent).mirrorY(progressZone.middleY()), progressColor);
        }
        ...
        if (chosenTxt != null) {
            ...
            TextUtil.render2DText(guiGraphics, chosenTxt.withStyle(true, InvoTextFormat.filter(ChatFormatting.BOLD)).getText(), true,3,
                    headerZone.inflate(-2,-3), TextUtil.txtAlignment.MIDDLE);
        }
    }
    public static void renderTimer(GuiGraphics guiGraphics, FallenData cap, InvoZone workZone, float y, float progressCircleRadius, int imageSize){
        ...
        //This is overkill
        CircleRender.draw2DArc(guiGraphics, timerZone.middleX(), timerZone.middleY(), (progressCircleRadius + 4) * progressCircleMultiplier,
                0, Math.max(0.001D,cap.getOverkillPercentage() * 360), purpleProgCircle);

        //This is overheal
        CircleRender.draw2DArc(guiGraphics, timerZone.middleX(), timerZone.middleY(), (progressCircleRadius + 2) * progressCircleMultiplier,
                0, Math.max(0.001D,cap.getOverhealPercentage() * 360), goldProgCircle);
//        guiGraphics.guiRenderState.forEachElement(state -> LOGGER.error(state.getClass().getName()), GuiRenderState.TraverseRange.ALL);
...
//                ", W:"+(90*2)+", H:"+(90*2));
//        ClientUtil.blit2DColor(guiGraphics, InvoZone.fromPoint(timerZone.middle()).inflate(45), Color.BLACK.getRGB());
        CircleRender.draw2DArc(guiGraphics, timerZone.middleX(), timerZone.middleY(), progressCircleRadius * progressCircleMultiplier, 0, endAngle, greenColor);
//        ClientUtil.getMinecraft().renderBuffers().bufferSource().endBatch();
        ...
        timerIMG.render(guiGraphics);
        timerIMG.getRenderZone().copy(originalZone);

        TextUtil.render2DText(guiGraphics, timeLeftTxt.getText(), false, 1,
