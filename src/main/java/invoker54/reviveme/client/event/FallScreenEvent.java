    ...
    public static void registerFallenScreen(){
        ...
        OverlayRegistry.registerOverlayAbove(ForgeIngameGui.CHAT_PANEL_ELEMENT, "fallen_screen", (gui, stack, partialTicks, width, height) ->
        {
            ...
            MutableComponent timeLeftString =
                    new TextComponent((seconds <= 0) ? "INF" : Integer.toString((int) seconds))
        ...
        OverlayRegistry.registerOverlayAbove(ForgeIngameGui.CHAT_PANEL_ELEMENT, "fallen_self_revive_screen", (gui, stack, partialTicks, width, height) ->
        {
            ...
            if (!cap.usedChance()) {
                ...
                stringToRender = new TranslatableComponent("fallenScreen.self_revive.give_chance_1");
            stringToRender = new TextComponent(stringToRender.getString().replace("{attack}",inst.options.keyAttack.getKey().getDisplayName().getString()));
                ...
                ClientUtil.blitColor(stack, halfWidth / 4F, (halfWidth / 2F), startHeight + (halfWidth / 2F),
                        20, new Color(0, 0, 0, 182).getRGB());
                stringToRender = new TranslatableComponent("fallenScreen.self_revive.give_chance_2");
                TextUtil.renderText(stack, stringToRender.withStyle(ChatFormatting.BOLD), true, halfWidth / 4, (halfWidth / 2F),
            ...
            stringToRender = new TranslatableComponent("fallenScreen.self_revive.give_items");
            stringToRender = new TextComponent(stringToRender.getString().replace("{use}",inst.options.keyUse.getKey().getDisplayName().getString()));
            ...
            for (Item item : itemArrayList) {
                ...
                MutableComponent countComp = new TextComponent("" + count).withStyle(ChatFormatting.BOLD).withStyle(ChatFormatting.GREEN);
                MutableComponent arrowComp = new TextComponent(" -> ").withStyle(ChatFormatting.BOLD);
                MutableComponent newCountComp =
                        new TextComponent("" + (count - (Math.round(Math.max(1, ReviveMeConfig.sacrificialItemPercent * count)))))
            ...
            if (cap.usedSacrificedItems() || cap.getItemList().size() == 0){
                ...
                MutableComponent item_text = new TranslatableComponent(cap.usedSacrificedItems() ? "fallenScreen.self_revive.used" : "fallenScreen.self_revive.no_items");
            ...
            if (cap.usedChance()){
                ...
                TextUtil.renderText(stack, new TranslatableComponent("fallenScreen.self_revive.used").withStyle(ChatFormatting.DARK_RED), true,
        ...
        OverlayRegistry.registerOverlayAbove(ForgeIngameGui.CHAT_PANEL_ELEMENT, "fallen_self_revive_compact_screen", (gui, stack, partialTicks, width, height) ->
        {
            if (mC.screen instanceof ChatScreen) return;
            if (!ReviveMeConfig.compactReviveUI) return;
            if (!ReviveMeConfig.selfReviveMultiplayer &&
