    ...
    public static void renderReviveOption(int mouseButton, PoseStack stack, InvoZone workZone, FallenData cap, boolean beingHeld) {
        ...
        switch (selfReviveType) {
            ...
            case SPECIFIC_ITEM: {
                ...
                InvoText badText = InvoText.translate("revive_me.fallenScreen.self_revive.specific_item.false_1")
                ...
                if (itemPair.getKey() >= ReviveMeConfig.specificItemCount) {
                    ...
                    for (int a = 0; a < inventory.getContainerSize(); a++) {
                        ...
                        if (!ItemStack.isSameItem(specificStack, containerStack)) continue;
                        if (!ItemStack.isSameItemSameComponents(specificStack, containerStack)) continue;
...
//                    ClientUtil.blitColor(stack, mainZone, new Color(0,0,0,230).getRGB());
            ...
            case STATUS_EFFECTS: {
                ...
                for (MobEffect effect : cap.getNegativeStatusEffects()) {
                    ...
                    textZone.setX(textZone.right()).setWidth(textZone.width() * 3).inflate(-2, -5)
                            .splitHeight(2, 1).shift(0,-1);
