...
public class RevivePlayerActionsEvent {
    ...
    public static void reviveCheck(TickEvent.PlayerTickEvent event){
        ...
        if (!cancelEvent) {
//            //System.out.println("Someone I'm reviving? : " + (FallenCapability.get((Player)inst.crosshairPickEntity).
//                    compareUUID(myUUID)));
...
//                    isReviver(myUUID));
            cancelEvent = (inst.crosshairPickEntity != targPlayer && (ReviveMeConfig.reviverMustLook ||
                    ClientUtil.getPlayer().distanceTo(targPlayer) > ClientUtil.getPlayer().getAttributeValue(ForgeMod.REACH_DISTANCE.get())));
        }
    ...
    public static void reviveItemUse(TickEvent.PlayerTickEvent event) {
        ...
        if (!(inst.crosshairPickEntity instanceof PlayerEntity)) return;

        FallenCapability cap = FallenCapability.get((PlayerEntity) inst.crosshairPickEntity);
