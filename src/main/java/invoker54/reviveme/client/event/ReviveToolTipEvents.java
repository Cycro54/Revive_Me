...
//        }
        ...
        List<MobEffectInstance> instanceList = itemData.getReviveEffects();

        int fallenAmp = FallenData.get(ClientUtil.getPlayer()).getPenaltyMultiplier();
        instanceList.add(new MobEffectInstance(MobEffectInit.FALLEN_EFFECT, (int)(itemData.getFallenPenaltyTimer() * 20), fallenAmp));

        for (MobEffectInstance instance : itemData.getReviveEffects()){
            ...
            InvoText name = InvoText.component((MutableComponent) instance.getEffect().value().getDisplayName());
