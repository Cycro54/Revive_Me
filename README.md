...
* Stop Revive Me! sounds if player is dead (check if player is dead...)
...
* (Config) Add a blacklist to Random item sacrifice
* (Config) Add option to kill player even when they are revived, but to respawn them exactly where they die
* (Config) Add a way for certain types of damage sources to bypass my mod
